...
// Get product reviews
$sql = "SELECT r.id, r.product_id, r.user_id, r.rating, r.comment, r.created_at as user_name 
        FROM reviews r 
...
if ($stmt) {
    ...
    if ($stmt->execute()) {
        ...
        $reviews = false; // Set to false if execution fails
        error_log("Error executing reviews query: " . $stmt->error);
    }
} else {
    $reviews = false; // Set to false if prepare fails
...
<head>
    ...
    <title><?php echo htmlspecialchars($product['name']); ?> - KR's Tech</title>
...
:root {
        ...
        * {
        ...
        .add-to-cart-btn, .product-card-btn {
            background-color: #2ecc71;
            transition: background-color 0.3s;
        }
        
        .add-to-cart-btn:hover, .product-card-btn:hover {
            background-color: #27ae60;
        }
        
        /* Update footer styles to match index.php */
        footer {
    ...
    <header>
        <div class="container">
            <div class="header-content">
                <a href="index.php" class="logo">KR's<span>Tech</span></a>
                <nav>
                    <ul>
                ...
                <?php if ($related_products && $related_products->num_rows > 0): ?>
                    ...
                    <div class="product-card">
                        ...
                        <div class="product-card-info">
                            ...
                            <p class="product-card-price">R<?php echo number_format($related['price'], 2); ?></p>
...
<script>
    ...
    // Function to update cart count with retry logic
    ...
    // Generic function to handle add to cart operations
    async function addToCart(productId, quantity, productName, buttonElement = null) {
        if (!productId || !quantity) return false;
        
        // Disable button during operation if provided
        if (buttonElement) {
    ...
    if (mainAddToCartBtn) {
        mainAddToCartBtn.addEventListener('click', async function() {
            ...
            await addToCart(productId, quantity, productName, this);
        });
    }
    ...
    document.querySelectorAll('.product-card-btn').forEach(button => {
        button.addEventListener('click', async function(e) {
            ...
            const productId = this.getAttribute('data-id');
            const productName = this.closest('.product-card-info').querySelector('.product-card-title').textContent;
            
            await addToCart(productId, 1, productName, this);
        });
    });
