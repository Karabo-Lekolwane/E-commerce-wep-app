...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_user'])) {
    ...
    if (!empty($_POST['password'])) {
        $stmt->bind_param("sssssssssssssssi", 
            ...
            $card_name, $card_last_four, $card_expiry, $card_type,
            $password, $user_id);
    } else {
    ...
    if ($stmt->execute()) {
        ...
        $user_error = "Error updating user: " . $stmt->error;
    }
    $stmt->close();
}

// Handle user deletion (only for non-admin users)
if (isset($_GET['delete_user']) && is_numeric($_GET['delete_user'])) {
    $user_id = $_GET['delete_user'];
    
    // First check if the user is not an admin
    $check_admin = "SELECT is_admin FROM users WHERE id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($user['is_admin'] == 0) {
            ...
            if ($delete_stmt->execute()) {
                ...
                $user_error = "Error deleting user: " . $conn->error;
            }
            $delete_stmt->close();
        } else {
            $user_error = "Cannot delete admin users";
        }
    } else {
...
if (isset($_GET['toggle_admin']) && is_numeric($_GET['toggle_admin'])) {
    ...
    if ($user_id != $_SESSION['user_id']) {
        ...
        $admin_error = "You cannot modify your own admin status";
    }
}
        ...
        <div class="admin-content">
            <div class="card">
                                    ...
                                    <?php if ($products->num_rows > 0): ?>
                                        <?php while ($product = $products->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td><b>R </b><?php echo number_format($product['price'], 2); ?></td>
                ...
                <div id="users" class="tab-content">
                    ...
                    <!-- User Edit Form Card (only shown when editing) -->
<?php if ($edit_user): ?>
<div class="card">
    <div class="card-header">
        <h2 class="card-title">Edit User: <?php echo htmlspecialchars($edit_user['username']); ?></h2>
    </div>
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="state" name="state" 
                       value="<?php echo htmlspecialchars($edit_user['state']); ?>">
            </div>
            
            <div class="form-group">
                <label for="zip">ZIP Code</label>
                <input type="text" class="form-control" id="zip" name="zip" 
                       value="<?php echo htmlspecialchars($edit_user['zip']); ?>">
            </div>
            
            <div class="form-group">
                <label for="country">Country</label>
                <input type="text" class="form-control" id="country" name="country" 
                       value="<?php echo htmlspecialchars($edit_user['country']); ?>">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="card_name" name="card_name" 
                       value="<?php echo htmlspecialchars($edit_user['card_name']); ?>">
            </div>
            
            <div class="form-group">
                <label for="card_last_four">Card Last Four</label>
                <input type="text" class="form-control" id="card_last_four" name="card_last_four" 
                       value="<?php echo htmlspecialchars($edit_user['card_last_four']); ?>">
            </div>
            
            <div class="form-group">
                <label for="card_expiry">Card Expiry</label>
                <input type="text" class="form-control" id="card_expiry" name="card_expiry" 
                       value="<?php echo htmlspecialchars($edit_user['card_expiry']); ?>">
            </div>
            
            <div class="form-group">
                <label for="card_type">Card Type</label>
                <input type="text" class="form-control" id="card_type" name="card_type" 
                       value="<?php echo htmlspecialchars($edit_user['card_type']); ?>">
            </div>
        </div>
        
        <div style="margin-top: 20px;">
            <button type="submit" name="submit_user" class="btn btn-success">Update User</button>
            <a href="admin.php" class="btn btn-primary" style="margin-left: 10px;">Cancel</a>
        </div>
    </form>
</div>
<?php endif; ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <?php endif; ?>
                                                                                                ...
                                                                                                <td class="actions">
                                                    <a href="admin.php?toggle_admin=<?php echo $user['id']; ?>#users" class="btn btn-primary">
                                                        <?php echo $user['is_admin'] ? 'Remove Admin' : 'Make Admin'; ?>
                                                    </a>
                                                    <a href="admin.php?edit_user=<?php echo $user['id']; ?>#users" class="btn btn-primary">Edit</a>
                                                    <?php if ($user['is_admin'] == 0): ?>
                                                        <a href="admin.php?delete_user=<?php echo $user['id']; ?>#users" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this user? This action cannot be undone.')">
                                                            Delete
                                                        </a>
                                                    <?php endif; ?>
                                                </td>
    ...
    <button onclick="window.location.href='account.php'" style="padding: 15px 20px; background-color: #4CAF50; color: white; border: none; border-radius: 10px; cursor: pointer;">
        Go to Account
    </button>
</div>
